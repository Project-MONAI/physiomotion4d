...
class TestSegmentChestVista3D:
    ...
    def test_segmenter_initialization(self, segmenter_vista_3d):
        ...
        print(
            f"  Soft tissue structures: {len(segmenter_vista_3d.soft_tissue_mask_ids)}"
        )
        print(f"  Bundle path: {segmenter_vista_3d.bundle_path}")

    def test_segment_single_image(
        self, segmenter_vista_3d, test_images, test_directories
    ):
    ...
    def test_label_prompt_segmentation(
        self, segmenter_vista_3d, test_images, test_directories
    ):
    ...
    def test_contrast_detection(self, segmenter_vista_3d, test_images):
        ...
        result_no_contrast = segmenter_vista_3d.segment(
            input_image, contrast_enhanced_study=False
        )
        contrast_mask_no = result_no_contrast["contrast"]

        # Test with contrast flag
        result_with_contrast = segmenter_vista_3d.segment(
    ...
    def test_set_and_reset_prompts(self, segmenter_vista_3d):
        ...
        assert segmenter_vista_3d.label_prompt == [
            115,
            6,
        ], "Label prompt not set correctly"
