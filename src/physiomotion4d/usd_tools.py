...
def load_openusd_as_vtk(
    usd_file: str | Path,
    prim_path: str = "/World",
    time_code: float | None = None,
) -> pvtk.PolyData:
    """Load OpenUSD mesh geometry as a PyVista ``PolyData``.

    The loader evaluates mesh points at ``time_code``, applies each mesh prim's
    local-to-world transform, and stores RGB colors in ``point_data['openusd_rgb']``.
    Authored ``displayColor`` is used when available; otherwise points are colored
    red. Coordinates are returned in the USD stage coordinate system.

    Args:
        usd_file: Path to an OpenUSD file.
        prim_path: Root prim path to traverse. Defaults to ``/World``.
        time_code: Optional time code for animated meshes. ``None`` reads default
            values and falls back to the first authored point time sample.
    ...
    if not usd_path.exists():
        raise FileNotFoundError(f"OpenUSD file not found: {usd_path}")

    stage = Usd.Stage.Open(str(usd_path))
    if stage is None:
        raise ValueError(f"Could not open OpenUSD file: {usd_path}")

    root_prim = stage.GetPrimAtPath(prim_path)
    if not root_prim.IsValid():
        raise ValueError(f"OpenUSD prim path not found: {prim_path}")
    ...
    for prim in Usd.PrimRange(root_prim):
        ...
        for count in face_counts:
            ...
            faces.extend(int(face_indices[index_offset + i]) for i in range(count_int))
            index_offset += count_int
        if not faces:
            continue

        pv_mesh = pvtk.PolyData(points, np.asarray(faces, dtype=np.int64))
        rgb = _openusd_display_color(mesh, len(points), usd_time)
